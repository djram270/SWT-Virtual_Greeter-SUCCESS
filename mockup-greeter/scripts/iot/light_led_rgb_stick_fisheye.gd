# res://scripts/iot/light_led_rgb_stick_fisheye.gd
"""
Light led rgb stick fisheye monitor array Display Controller
- Handles only visual representation (no control logic)
- Real device control is managed by VirtualGreeter through WebSocket
"""
extends Node3D
# --- Configuration ---
@export var entity_id: String = "light.led_rgb_stick_fisheye"
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and event.keycode == Key.KEY_7:
...
#description label 
@export var description := "This is a programmable RGB lighting system that enhances the room’s atmosphere. You can turn off and turn on this lamp with key [7]."

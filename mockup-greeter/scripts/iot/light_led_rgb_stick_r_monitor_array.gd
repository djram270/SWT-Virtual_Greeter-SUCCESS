# res://scripts/iot/light_led_rgb_stick_r_monitor_array.gd
"""
Light led rgb stick r monitor array Display Controller
- Handles only visual representation (no control logic)
- Real device control is managed by VirtualGreeter through WebSocket
"""
extends Node3D
# --- Configuration ---
@export var entity_id: String = "light.led_rgb_stick_r_monitor_array"
...
func _input(event: InputEvent) -> void:
	"""Detect manual toggle key"""
	if event is InputEventKey and event.pressed and event.keycode == Key.KEY_9:
...
#descrption label 
@export var description := "This is a programmable RGB lighting system that enhances the room’s atmosphere. You can turn off and turn on this lamp with key [9]."
